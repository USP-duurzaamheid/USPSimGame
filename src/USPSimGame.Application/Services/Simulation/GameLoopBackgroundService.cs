        ...
        foreach (var session in activeSessions)
        {
            if (nowUtc >= session.TargetMonthEndUtc!.Value)
            {
                ...
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    var costService = scope.ServiceProvider.GetRequiredService<USPSimGame.Application.Services.Costing.ICostCalculationService>();
                    var teamBudgetService = scope.ServiceProvider.GetRequiredService<USPSimGame.Application.Services.Costing.ITeamBudgetService>();
                    var teamNotifier = scope.ServiceProvider.GetRequiredService<USPSimGame.Application.Services.ITeamNotifierService>();

...
namespace USPSimGame.Domain.Utils;
...
public static class GeoJsonSpatialUtils
{
    ...
    public static double CalculateFeatureQuantity(string? geoJson, Enums.PlannableGeometryType geomType)
    {
        ...
        try
        {
            ...
            if (geomType == Enums.PlannableGeometryType.Point)
            {
                return CountPointsInGeoJson(root);
            }
            else if (geomType == Enums.PlannableGeometryType.Line)
            {
                double totalMeters = CalculateTotalLineLengthMeters(root);
                return Math.Max(1.0, Math.Round(totalMeters / 50.0, 1));
            }
            else if (geomType == Enums.PlannableGeometryType.Polygon)
            {
